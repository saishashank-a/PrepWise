  ...
  return (
    <div className="h-full flex flex-col rounded-xl border border-border-subtle overflow-hidden">
      <div className="px-3 py-2 bg-surface-elevated border-b border-border-subtle flex items-center gap-2">
        ...
        <span className="text-[10px] font-medium text-text-dim uppercase tracking-wider">Output</span>
      </div>
        ...
        {hasOutput ? (
          <pre
            className={`text-xs font-mono leading-relaxed whitespace-pre-wrap break-words ${
              error ? "text-red-400" : "text-emerald-glow/80"
            }`}
          >
            {error ? `Error: ${error}` : output}
          </pre>
        ) : (
          <p className="text-xs text-text-dim italic">Run your code to see output here.</p>
        )}
      </div>
    </div>
  );
