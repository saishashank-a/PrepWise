  ...
  if (resumeText) {
    return (
      <div className="space-y-3">
        ...
        <div className="max-h-32 overflow-y-auto rounded-xl bg-surface-elevated border border-border-default p-4 text-sm text-text-secondary leading-relaxed">
          {resumeText.slice(0, 800)}
          {resumeText.length > 800 && (
            <span className="text-text-muted">... ({resumeText.length} chars)</span>
          )}
        </div>
      </div>
    );
      ...
      <div
        ...
        className={`relative rounded-xl border-2 border-dashed p-8 text-center cursor-pointer transition-all duration-300
          ${isDragActive
            ...
            : "border-border-default hover:border-primary-border hover:bg-surface"
          }
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted mt-1">PDF or DOCX — skills auto-extracted</p>
          </div>
        </div>
      </div>
