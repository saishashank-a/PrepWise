...
export default function ResumeUpload() {
  ...
  const { resumeText, setResumeText } = useProfileStore();
  ...
  if (resumeText) {
    return (
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-emerald-glow/10 border border-emerald-glow/20 flex items-center justify-center">
              <svg
                className="w-4 h-4 text-emerald-glow"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.5}
        ...
        <div className="max-h-48 overflow-y-auto rounded-xl bg-surface-elevated border border-border-subtle p-4 text-sm text-text-muted leading-relaxed">
          {resumeText.slice(0, 1000)}
          {resumeText.length > 1000 && (
            <span className="text-text-dim">... ({resumeText.length} characters total)</span>
          )}
        </div>
      </div>
    );
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-emerald-glow/10 border border-emerald-glow/20 flex items-center justify-center">
            {parsing ? (
              <svg
                className="w-5 h-5 text-emerald-glow animate-spin"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
              ...
              <svg
                className="w-5 h-5 text-emerald-glow"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.5}
          ...
          <div>
            ...
            <p className="text-xs text-text-dim mt-1">PDF or DOCX — parsed entirely in your browser</p>
          </div>
        </div>
      </div>
      {error && (
